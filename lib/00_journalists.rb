def initialisation
  puts "Salut vieux loup et bienvenue dans LE GAME !!"
  puts "Que veux-tu savoir sur notre liste de journaliste ? (press la touche correspondante)"
  puts ""
  puts "1 : Combien y a-t-il de journalistes dans ce array ? "
  puts "2 : Combien de journalistes contiennent un numéro dans leur blazzzze ? ? "
  puts "3 : Combien de journaliste ont les 4 lettres du prénom Aude à la suite (sans prendre en compte les majuscules) dans leur blazz ? "
...
def jeu(choice)
  ...
  #puts journalists.count{|handle| handle.grep?(/Integer/)}
    puts ""
    perform

  elsif choice == 3 
  # Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans prendre en compte les majuscules) ?
    puts journalists.count {|handle| handle.include?("aude") || handle.include?("Aude")}
    puts ""
    perform

  elsif choice == 4 
  # Combien commencent par une majuscule (première lettre juste après le @) ?
    puts journalists.count{|handle| handle[1].match?( /[[:upper:]]/ )} 
    puts ""
    perform

  elsif choice == 5 
  # Combien contiennent au moins une majuscule ?
    puts journalists.count{|handle| handle.match?( /[[:upper:]]/ )} 
    puts ""
    perform

  elsif choice == 6 
  # Combien y a-t-il de underscore _ dans tous les pseudos confondus ?
    puts journalists.count {|handle| handle.include?("_")}
    puts ""
    perform
  ...
  # Quels sont les 50 handles les plus courts de ce array ?
    journalists.min(50) {|a, b| a.length <=> b.length }
    puts ""
    perform
